...
def add_arguments(argument_parser: argparse.ArgumentParser) -> None:
    ...
    argument_parser.add_argument('-a', dest='reg_addr', metavar='<reg_addr>',
                                 help="Addr of the reg to filter")  # TODO add type
    ...
    argument_parser.add_argument('-s', action='store_true', dest='print_summary', help='Print summary')
...
class DataByte():
    ...
    def __str__(self):
        #return f"DataByte(data_in={self.data_in}, data_out={self.data_out}, time={self.time}, line={self.line})"
                    ...
                    if enable_detected:
                        if first_frame:
                            if splitted_line[4] == self.addr_filter:
                                self.current_frame.append(DataByte(data_in = splitted_line[4], data_out = splitted_line[5], time = splitted_line[2],
                                              line=line_number))
                                addr_detected = True
                            first_frame = False
                        else:
